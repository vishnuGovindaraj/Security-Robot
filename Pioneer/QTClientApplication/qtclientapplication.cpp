...
public:
    ...
    void run() {
		//Creates ARIA client in separate thread
		QTClientApplication::runClient();
    }
};
...
QTClientApplication::QTClientApplication(QWidget *parent)
	: QMainWindow(parent)
{
	ui.setupUi(this);
	//Constructor
...
QTClientApplication::~QTClientApplication()
{
	//Destructor
}
...
void QTClientApplication::on_upButton_pressed()
{
	upPressed = true;

}
void QTClientApplication::on_upButton_released()
{
	upPressed = false;
}
void QTClientApplication::on_downButton_pressed()
{
	downPressed = true;
}
void QTClientApplication::on_downButton_released()
{
	downPressed = false;
}
void QTClientApplication::on_leftButton_pressed()
{
	leftPressed = true;
}
void QTClientApplication::on_leftButton_released()
{
	leftPressed = false;
}
void QTClientApplication::on_rightButton_pressed()
{
	rightPressed = true;
}
void QTClientApplication::on_rightButton_released()
{
	rightPressed = false;
}

void QTClientApplication::on_connectButton_clicked()
{
	std::cout << "UP\n";

	

	clientThread = new MyThread(10,"Thread 01");
		try{
        clientThread->start();
...
void QTClientApplication::on_disconnectButton_clicked()
{
...
void QTClientApplication::runClient(){
	...
	string portNumber = "7272";

	
	char* argv[4] = {"-host", "192.168.137.1", "-port", "7272"};
	int argc = 4;
...
/* Aria initialization: */
  ...
  while (client.getRunningWithLock())
  {
    ...
    ArUtil::sleep(100);
	if (upPressed){iHandler->up(); }
	if (downPressed){iHandler->down(); }
	if (leftPressed){iHandler->left(); }
	if (rightPressed){iHandler->right(); }
  }
